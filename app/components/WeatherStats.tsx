...
export default function WeatherStats({ weatherData, loading }: WeatherStatsProps) {
  ...
  return (
    <View className="w-full max-w-sm">
      <View className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
        <Text className="text-white text-lg font-semibold mb-4">Weather Details</Text>
        
        {stats.map((stat, index) => (
          <View key={index} className="mb-4 last:mb-0">
            <View className="flex-row justify-between items-start">
              <View className="flex-1">
                <Text className="text-white/60 text-sm mb-1">{stat.title}</Text>
                <Text className="text-white text-base font-medium">{stat.value}</Text>
                <Text className="text-white/40 text-xs mt-1">{stat.subtitle}</Text>
              </View>
            </View>
